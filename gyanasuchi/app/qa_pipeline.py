...
from gyanasuchi.common import env, setup_logging
...
class QuestionAnswerPipeline:
    def __init__(self, collection_name, embeddings_model='sentence-transformers/all-MiniLM-L6-v2'):
    ...
    def load_language_model(temperature=0) -> ChatOpenAI:
        return ChatOpenAI(openai_api_key=env("OPENAI_API_KEY"),
    ...
    @staticmethod
    def generate_text_chunks(documents: list, chunk_size: int = 500, chunk_overlap: int = 10,
                             splitter=RecursiveCharacterTextSplitter) -> list:
    ...
    def load_embeddings(self, device: str = 'cpu') -> HuggingFaceEmbeddings:
        return HuggingFaceEmbeddings(model_name=self.embeddings_model, model_kwargs={'device': device})

    def create_qdrant_database(self, documents: list, embeddings: HuggingFaceEmbeddings,
                               recreate_collection: bool = True) -> Qdrant:
