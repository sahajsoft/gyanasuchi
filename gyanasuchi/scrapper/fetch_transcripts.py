...
def fetch_transcripts_if_not_in_db(
    video_id: str,
    run_id: datetime,
) -> List[YouTubeTranscriptLine]:
    transcripts_from_db = YouTubeTranscriptLine.select().where(
        YouTubeTranscriptLine.video == video_id,
    )

    if transcripts_from_db:
        logger.warning(
            f"Deleting {len(transcripts_from_db)} for {video_id=} because transcripts already exists",
        )
...
def process_video(video_id: str, run_id: datetime) -> TranscriptUpdateStatus:
    ...
    logger.info(
        f"Inserting {len(lines)} transcript lines for {video_id=} with {batch_size=}",
    )
    ...
    YouTubeVideo.update({"fetched_transcripts_at_run": run_id}).where(
...
def main() -> None:
    ...
    videos_without_transcripts = YouTubeVideo.select().where(
        YouTubeVideo.fetched_transcripts_at_run.is_null(),
    )

    logger.info(
        f"fetched {len(videos_without_transcripts)} videos which need transcripts",
    )
    logger.info(
        [process_video(video.id, run_id) for video in videos_without_transcripts],
    )
